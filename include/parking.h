 ...
 * `Parkhaus` if there is space. Sets the `Car` structs time remaining and time of entry variables
 * and reduces the empty_spaces count of the `Parkhaus` struct by one.
 ...
 * @param[in] current_time Current time (time of entry)
 * @param[in] remaining_time Duration the `Car`will spend in the Parkhaus
 * @return int Status code:
 *            - `0` if the operation succeeded.
 *            - `1` if there are no free spaces
 *             - `-1` if an error occurred (e.g., pointer is `NULL`).
 */
int park_car(Parkhaus *p_parkhaus, Car *p_car, int current_time, int remaining_time);
